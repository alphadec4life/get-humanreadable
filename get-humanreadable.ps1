#function dir-subdirs{
#    [CmdletBinding()]
#    [Alias("subdirs")]
#    Param(
#        [parameter(valuefrompipeline=$true)]
#        [System.IO.Directory]$parent_dir,
#        [string]$dir_children
#    )
#}
function get-humanreadable(){
    ...
    Param(
        [parameter(valuefrompipeline=$true)]
        [string]$bytecount, # = [system.convert]::ToInt64($bytecount) i tried to convert string to int but no luck
        [string]$directory,
...
for($i=0;$i -lt $gci_DIR_Sum.Count;$i++){
    $gci_DIR_Sum_UPDATED += @{
        $gci_DIR_Sum[$i].GetEnumerator() | select -expand .keys[$i] = [system.convert]::ToInt64($gci_DIR_Sum.values[$i])
    }
}
...
for($i=0;$i -le $t.count;$i++){
    $DIR_2_INT += @{
        $t.name[$i] = [system.convert]::ToInt64($t.Value[$i])
    }
}
...
$DIR_2_INT = @{}
$t = ipcsv $report\dirsize_byte.csv
for($i=0;$i -le $t.count;$i++){
    $DIR_2_INT += @{
        $t.name[$i] = [system.convert]::ToInt64($t.Value[$i])
    }
...
$gci_files_Screen = $DIR_2_INT.GetEnumerator() | select name,value | sort value -Descending | select -First 10
...
$DIR_2_INT.GetEnumerator() | sort {$_.value} -Descending | select @{n="$server : '$directory'";e="name"},@{n="size sorted on: $HourMinDATE";e={cdwtreesize $_.value}} | ConvertTo-Csv -NoTypeInformation | Out-File $report\dir_size_SORTED.csv -Force
